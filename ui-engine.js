...
(() => {
  ...
  function statClass(val) {
    return val && val !== '[N/A]' ? 'status-live' : 'status-muted';
  }
  ...
  function statcastFactorLine(label, value) {
    const safeValue = value || '[N/A]';
    return `<div class="${statClass(safeValue)}">${escapeHtml(label)}: ${escapeHtml(safeValue)}</div>`;
  }
  ...
  function summarizeBranchFactors(key, parsed) {
    ...
    if (key === 'A') {
      return order.map(label => {
        if (['xba','xwoba','xslg','avgExitVelocity','whiffPct'].includes(label)) {
          const map = { xba: 'xBA', xwoba: 'xwOBA', xslg: 'xSLG', avgExitVelocity: 'Avg Exit Velo', whiffPct: 'Whiff%' };
          return statcastFactorLine(map[label], parsed[label]);
        }
        return factorLine(label, parsed[label]);
      }).join('') || '<div class="mini-muted">No parsed factors.</div>';
    }

...
(() => {
  const SYSTEM_VERSION = 'v13.8.0 (PROBE-MATCH)';
  ...
  async function handednessLock(personId, row = {}, scheduleCtx = {}) {
    try {
      const payload = await safeFetchJson(`https://statsapi.mlb.com/api/v1/people/${personId}`, { timeout: 8000 });
...
async function branchA(row, scheduleCtx) {
  ...
  try {
    const profile = await safeFetchJson(`https://statsapi.mlb.com/api/v1/people/${identity.personId}`, { timeout: 8000 });
  ...
  async function fetchPayload(url) {
    ...
    const isProfileProbe = /\/people\/\d+(?:\?|$)/.test(url);
    const isStatcastProbe = /stats=(?:expectedStatistics|statcast50ft)/.test(url);
    const payload = await safeFetchJson(url, { timeout: (isProfileProbe || isStatcastProbe) ? 8000 : 3000 });
  ...
  function mapBatteryPayload(payload, groupName) {
    ...
    const room = StatcastRoomFinder(results, groupName) || findGroupStat(results, groupName) || {};
    const stat = room?.splits?.[0]?.stat || {};
    const f = {};

    // TARGET: 2026 Key-Mapping & Promotion
    (results.stats || []).forEach(group => {
      const gName = group.group?.name || group.group?.displayName;
      const s = group.stats || {};
      if (gName === 'expectedStatistics' || gName === 'expectedStats') {
        // Map both standard and nested API variants
        f.xba = s.avg?.xba || s.xba || f.xba;
        f.xwoba = s.avg?.xwoba || s.xwoba || f.xwoba;
        f.xslg = s.avg?.xslg || s.xslg || f.xslg;
      }
      if (gName === 'statcast50ft' || gName === 'statcastMetrics') {
        f.avgExitVelocity = s.avg?.exitVelocity || s.exitVelocity || f.avgExitVelocity;
        f.whiffPercentage = s.avg?.whiffPercentage || s.whiffPercentage || f.whiffPercentage;
      }
    });

    if (groupName === 'expectedStatistics' || groupName === 'expectedStats') {
      return {
        xba: pickNum(f.xba, stat?.xba, stat?.expectedBattingAverage, recursiveSeek(room, 'xba'), recursiveSeek(room, 'expectedBattingAverage'), recursiveSeek(results, 'xba'), recursiveSeek(results, 'expectedBattingAverage')),
        xwoba: pickNum(f.xwoba, stat?.xwoba, stat?.expectedWeightedOnBaseAverage, recursiveSeek(room, 'xwoba'), recursiveSeek(room, 'expectedWeightedOnBaseAverage'), recursiveSeek(results, 'xwoba'), recursiveSeek(results, 'expectedWeightedOnBaseAverage')),
        xslg: pickNum(f.xslg, stat?.xslg, stat?.expectedSlugging, recursiveSeek(room, 'xslg'), recursiveSeek(room, 'expectedSlugging'), recursiveSeek(results, 'xslg'), recursiveSeek(results, 'expectedSlugging'))
      };
    }
    if (groupName === 'statcast50ft' || groupName === 'statcastMetrics') {
      return {
        avgExitVelocity: pickNum(f.avgExitVelocity, stat?.avgExitVelocity, stat?.exitVelocity, recursiveSeek(room, 'avgExitVelocity'), recursiveSeek(room, 'exitVelocity'), recursiveSeek(results, 'avgExitVelocity'), recursiveSeek(results, 'exitVelocity')),
        barrelPct: pickNum(stat?.barrelPercentage, recursiveSeek(room, 'barrelPercentage'), recursiveSeek(results, 'barrelPercentage')),
        hardHitPct: pickNum(stat?.hardHitRate, recursiveSeek(room, 'hardHitRate'), recursiveSeek(results, 'hardHitRate')),
        whiffPct: pickNum(f.whiffPercentage, stat?.whiffPct, stat?.whiffPercentage, recursiveSeek(room, 'whiffPct'), recursiveSeek(room, 'whiffPercentage'), recursiveSeek(results, 'whiffPct'), recursiveSeek(results, 'whiffPercentage')),
        velocityDelta: pickNum(stat?.velocityDelta, recursiveSeek(room, 'velocityDelta'), recursiveSeek(results, 'velocityDelta'))
      };
    }
    return {};
  }
  ...
  async function runOxyBattery(year) {
    ...
    const pId = identity.personId;
    // TARGET: Multi-Season Log Join (v2.0)
    const fetchLogs = async (yr) => {
      const url = `https://statsapi.mlb.com/api/v1/people/${pId}/stats?stats=gameLog&season=${yr}&group=pitching`;
      const res = await safeFetchJson(url, { timeout: 5000 });
      return res?.stats?.[0]?.splits || [];
    };
    const combinedLogs = [...(await fetchLogs(2026)), ...(await fetchLogs(2025)), ...(await fetchLogs(2024))];
    // Manual Hit Rate Calculation
    const l5 = combinedLogs.slice(0, 5).filter(g => Number(g.stat.strikeOuts || 0) > row.lineValue).length;
    const l10 = combinedLogs.slice(0, 10).filter(g => Number(g.stat.strikeOuts || 0) > row.lineValue).length;

    const liveAdvanced = deepMergeOxy(mapBatteryPayload(expectedRes.payload, 'expectedStats'), mapBatteryPayload(statcastRes.payload, 'statcastMetrics'));
    const pivotAdvanced = deepMergeOxy(mapBatteryPayload(expected25Res.payload, 'expectedStats'), mapBatteryPayload(statcast25Res.payload, 'statcastMetrics'));
    const fallbackAdvanced = deepMergeOxy(mapBatteryPayload(expected24Res.payload, 'expectedStats'), mapBatteryPayload(statcast24Res.payload, 'statcastMetrics'));
    ...
    ['xba','xwoba','xslg','avgExitVelocity','barrelPct','hardHitPct','whiffPct','velocityDelta'].forEach((key) => {
      const ordered = [
        ['expectedStats/statcastMetrics', liveAdvanced?.[key]],
        ['expectedStatsPrev1/statcastMetricsPrev1', pivotAdvanced?.[key]],
        ['expectedStatsPrev2/statcastMetricsPrev2', fallbackAdvanced?.[key]]
      ];
    ...
    const logs = combinedLogs.length
      ? combinedLogs.map(split => ({
          stat: split?.stat || {},
          isHome: Boolean(split?.isHome),
          dayNight: String(split?.game?.dayNight || '').toLowerCase(),
          gameDate: split?.date,
      ...
      : (liveLog.logs?.length ? liveLog.logs : (liveSeason.logs?.length ? liveSeason.logs : [ ...(season25.logs || []), ...(season24.logs || []) ]));
    const rates = calculateRates(seasonStats, logs, {
    ...
    if (propKey === 'strikeOuts') {
      trends.l5 = l5;
      trends.l10 = l10;
      trends.l20 = combinedLogs.slice(0, 20).filter(g => Number(g.stat.strikeOuts || 0) > row.lineValue).length;
      trends.samples = combinedLogs.length || trends.samples;
      trends.hitRateOverLine = trends.samples ? round2((trends.l20 / Math.min(20, trends.samples)) * 100) : trends.hitRateOverLine;
    }
...
async function branchB(row, scheduleCtx, opponentId = null) {
  ...
  // TARGET: High-Latency Profile Ingress
  if (!window.handCache) window.handCache = {};
  const pId = row.personId || row.mlbId || scheduleCtx?.personId || 0;
  let pitchHand = window.handCache[pId];

  if (!pitchHand) {
    const profile = await safeFetchJson(`https://statsapi.mlb.com/api/v1/people/${pId}`, { timeout: 8000 });
    pitchHand = profile?.people?.[0]?.pitchHand?.code || 'R';
    window.handCache[pId] = pitchHand;
  }
  const targetSplit = pitchHand === 'L' ? 'vs Left' : 'vs Right';
  const baseline = {
    opponentId: oppId || null, sourceYear: null, splitHandedness: pitchHand,
  ...
  try {
    ...
    const started = Date.now();
    const payload = await safeFetchJson(url, { timeout: 8000 });
    ...
    const handedEntry = splitEntries.find(sp => {
      const displayName = String(sp?.split?.displayName || sp?.split?.name || sp?.label || sp?.stat?.group?.displayName || '').trim();
      return displayName === targetSplit;
    }) || handednessMatcher(splitEntries, pitchHand);
